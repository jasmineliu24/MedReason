...
from dataset.config import transform_instruction_cot, transform_instruction_direct
...
def create_azure_batch_data(
    ...
    if "direct" in prompt_mode:
        ...
        #     dict_data["instruction"] = transform_instruction_direct(
        #         dict_data["instruction"]
        #     )
        # print(f"Transform the instruction to the Standard Q&A format")
        # print(f"Instruction: {dict_data['instruction']}")
    elif "cot" in prompt_mode:
        for dict_data in list_dict_data:
            dict_data["instruction"] = transform_instruction_cot(
                dict_data["instruction"]
            )
...
def prepare_azure_batch_data(
    ...
    for idx, dict_data in enumerate(list_dict_data):
        example_ids, list_message = get_formatted_chat(
            # example_ids, list_message = get_formatted_chat_ins_ex(
            tokenizer=tokenizer,
            language=language,
