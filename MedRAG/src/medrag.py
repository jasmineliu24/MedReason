...
class MedRAG:
    ...
    def __init__(self, llm_name="OpenAI/gpt-3.5-turbo-16k", rag=True, follow_up=False, retriever_name="MedCPT", corpus_name="Textbooks", db_dir="./corpus", cache_dir=None, corpus_cache=False, HNSW=False):
        ...
        print(f"Using vLLM for inference with model: {self.llm_name}")  # This line prints when model is initialized
        self.rag = rag
        ...
        self.templates = {"cot_system": general_cot_system, "cot_prompt": general_cot,
                    "medrag_system": general_medrag_system, "medrag_prompt": general_medrag, "no_cot": no_cot, "no_cot_system": no_cot_system}
        if self.llm_name.split('/')[0].lower() == "openai":
            ...
            # )
            # self.model = LLM(model=self.llm_name, tokenizer=self.tokenizer)
            # self.model = LLM(model=self.llm_name, tensor_parallel_size=4)
            # self.model = LLM(model=self.llm_name)
            print("Loading LLM")
            self.model = LLM(
                model=self.llm_name,
            ...
            # self.SamplingParams = SamplingParams(temperature=0.0, top_p=1, max_tokens=self.max_length)
            self.SamplingParams = SamplingParams(temperature=0.0, max_tokens=1024)
    ...
    def generate(self, messages, **kwargs):
        ...
        if "openai" in self.llm_name.lower():
            ...
            for output in response:
                ...
                ans.append(output.outputs[0].text)
        return ans
    ...
    def batch_answer(self, questions, options=None, k=32, rrf_k=100, save_dir = None, snippets=None, snippets_ids=None, **kwargs):
        '''
        ...
        for idx, (question, option) in enumerate(zip(questions, options)):
            ...
            #     option = '\n'.join([key+". "+option[key] for key in sorted(option.keys())])
            # else:
            #     option = ''
            ...
            if self.rag:
                if snippets is not None:
                    retrieved_snippets = snippets[idx][:k]
                    scores = []
                elif snippets_ids is not None:
                    if self.docExt is None:
                        self.docExt = DocExtracter(db_dir=self.db_dir, cache=True, corpus_name=self.corpus_name)
                    retrieved_snippets = self.docExt.extract(snippets_ids[idx][:k])
                    scores = []
                else:
                    assert self.retrieval_system is not None
                    retrieved_snippets, scores = self.retrieval_system.retrieve(question, k=k, rrf_k=rrf_k)
                    # idx as key, snippets as value
            ...
            # answers = []
            # if not self.rag:
            #     prompt_cot = self.templates["cot_prompt"].render(question=question, options=option)
            #     message = self.templates["cot_system"] + prompt_cot
            #     batch_prompt.append(message)
            
            # no cot version
            if not self.rag:
                prompt = self.templates["no_cot"].render(question=question, options=option)
                message = self.templates["no_cot_system"] + prompt
                batch_prompt.append(message)
                ...
                for context in contexts:
                    # prompt_medrag = self.templates["medrag_prompt"].render(context=context, question=question, options=option)
                    prompt_medrag = self.templates["medrag_prompt"].render(context=context, question=question, options=option)
                    message = self.templates["medrag_system"] + prompt_medrag
                    batch_prompt.append(message)
                    # messages=[
                    #         {"role": "system", "content": self.templates["medrag_system"]},
                    #         {"role": "user", "content": prompt_medrag}
        ...
        response = self.generate(batch_prompt, **kwargs)
        for res in response:
            answers.append(re.sub("\s+", " ", res))
        
        if self.rag:
            answers = [answers[i] for i in range(0, len(answers), len(contexts))]
        if save_dir is not None:
            with open(os.path.join(save_dir, "snippets.json"), 'w') as f:
                json.dump(retrieved_snippets, f, indent=4)
            with open(os.path.join(save_dir, "response.json"), 'w') as f:
